...
- v2.1 Sprint 2：新增 `tests/helpers.js` 與 `tests/master-verify.spec.js`；舊 `*-verify.spec.js` 定位為歷史 smoke spec，已被 master 72 steps 涵蓋，留作 sprint 級回歸性追蹤。
- RC1 cleanup：完成 P1-2 / P1-3 / P1-4 / P1-5 / P1-9；不包含 P1-1、P1-7、P1-8。
...
- 測試現況：`tests/master-verify.spec.js` 是 v2.1 主驗收入口，記錄完整 72 steps；舊 `sprint*-verify` / `phase-c` / `c1-c2` / `c3-c4-c5` spec 保留為歷史 smoke。
