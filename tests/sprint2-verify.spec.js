...
const SCREENSHOT_DIR = process.env.FORMA_SCREENSHOT_DIR || '/tmp/forma-sprint2-screenshots';
...
(async () => {
  ...
  await runStep('Test 2 v2 existing tabs smoke', page, async () => {
    ...
    for (const label of ['智慧製圖', 'Claude Design', 'NotebookLM', '提示詞庫', '體檢 & 增強', '彩蛋']) {
    ...
    expect(tabLabels.join(' | ').includes('智慧製圖 | 🎨 Claude Design | 📑 NotebookLM | 📚 提示詞庫 | 🩺 體檢 & 增強 | 彩蛋 🧪'), `tab 順序不符：${tabLabels.join(' | ')}`);
  ...
  await runStep('Test 25 StyleStudio remains absent while Audit is unlocked', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    ...
    for (const label of ['Claude Design', 'NotebookLM', '智慧製圖', '提示詞庫', '彩蛋']) {
      ...
      expect(!buttons.some(text => /StyleStudio/.test(text)), `${label} 不應出現 StyleStudio button：${buttons.join(' | ')}`);
    }
  });

  await runStep('Test 26 Audit tab visible and clickable in six-tab workflow', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    const tabs = await page.locator('header .border-t button').evaluateAll(nodes => nodes.map(n => n.textContent.trim()));
    expect(tabs.length === 6, `v2 應有 6 個 tab，實際 ${tabs.length}：${tabs.join(' | ')}`);
