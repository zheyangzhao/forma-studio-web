...
const SCREENSHOT_DIR = process.env.FORMA_SCREENSHOT_DIR || '/tmp/forma-sprint1-5-screenshots';
...
function verifyTranslationsJson() {
  ...
  const missing = [...ids].filter(id => !titleKeys.has(id));
  const extra = [...titleKeys].filter(id => !ids.has(id));

  expect(translations.schema_version === 2, `schema_version 應為 2，實際 ${translations.schema_version}`);
  ...
  expect(missing.length === 0, `translations titles missing: ${missing.join(', ')}`);
  expect(extra.length === 0, `translations titles extra: ${extra.join(', ')}`);
}
