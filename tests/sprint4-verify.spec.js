...
const SCREENSHOT_DIR = process.env.FORMA_SCREENSHOT_DIR || '/tmp/forma-sprint4-screenshots';
...
(async () => {
  ...
  await runStep('Test 2 v2 existing tabs smoke', page, async () => {
    ...
    for (const label of ['智慧製圖', 'Claude Design', 'NotebookLM', '提示詞庫', '體檢 & 增強', '風格實驗室', '彩蛋']) {
    ...
    expect(tabLabels.join(' | ').includes('🖼️ 智慧製圖 | 🎨 Claude Design | 📑 NotebookLM | 📚 提示詞庫 | 🩺 體檢 & 增強 | 🎬 風格實驗室 | 彩蛋 🧪'), `tab 順序不符：${tabLabels.join(' | ')}`);
  ...
  await runStep('Test 30 Sprint 1.8 hidden Audit actions are now visible', page, async () => {
    ...
    await clickText(page, '彩蛋');
    await clickText(page, '反 AI Slop 診斷');
    await page.locator('textarea').first().fill('一個 purple gradient generic card wall poster');
    await clickText(page, '執行實驗');
    await waitText(page, '前往 體檢 & 增強');
    await shot(page, 'sprint2-audit-actions-unlocked.png');
  });

  await runStep('Test 31 Audit onboarding card, weak sample, and dental placeholder', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickText(page, '體檢 & 增強');
    await waitText(page, '這個 tab 是做什麼的');
    ...
    expect(placeholder.includes('O2Win Dental') && placeholder.includes('植牙術後照護'), `placeholder 應為 O2Win 牙醫情境，實際 ${placeholder}`);
  ...
  await runStep('Test 32 seventh workflow includes Style Studio tab visible and clickable', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    const tabs = await page.locator('header .border-t button').evaluateAll(nodes => nodes.map(n => n.textContent.trim()));
    expect(tabs.length === 8, `應有 8 個 tab，實際 ${tabs.length}：${tabs.join(' | ')}`);
    expect(tabs.slice(0, 7).join(' | ') === '🖼️ 智慧製圖 | 🎨 Claude Design | 📑 NotebookLM | 📚 提示詞庫 | 🩺 體檢 & 增強 | 🎬 風格實驗室 | 彩蛋 🧪' && tabs[7].includes('設定'), `tab 順序不符：${tabs.join(' | ')}`);
    await page.locator('header button', { hasText: '風格實驗室' }).first().click();
    await waitText(page, '🎬 風格實驗室');
    await waitText(page, '5 類別 + 風格 chips');
