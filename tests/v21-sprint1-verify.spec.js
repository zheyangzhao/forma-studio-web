...
const SCREENSHOT_DIR = process.env.FORMA_SCREENSHOT_DIR || '/tmp/forma-v21-sprint1-screenshots';
...
(async () => {
  ...
  async function newIsolatedPage(viewport = { width: 1280, height: 900 }) {
    ...
    isolatedPage.on('pageerror', err => report.pageErrors.push(err.message));
    return { context: ctx, page: isolatedPage };
  }
  ...
  await runStep('Test 2 v2 existing tabs smoke', page, async () => {
    ...
    await waitText(page, '5 類別 + 風格 chips');
    await clickText(page, 'Claude Design');
    await clickText(page, '進階實驗工具');
    await waitText(page, '彩蛋 Lab');
    await clickText(page, '體檢 & 增強');
    await waitText(page, 'prompt 健檢工具');
  ...
  await runStep('Test 21 Smart selSub changes next-step actions', page, async () => {
    ...
    await waitText(page, '前往 Claude Design 精修');
    await waitText(page, '送去體檢');
    await clickText(page, '重新開始');
    await page.locator('textarea').first().fill('把這篇文獻整理成簡報');
    await clickText(page, '確認，生成提示詞');
    await waitText(page, '前往 NotebookLM tab 看完整流程');
    await waitText(page, 'NotebookLM 任務指令已產出');
  ...
  await runStep('Test 22 NotebookLM Step 5 paste order card visible', page, async () => {
    ...
    await waitText(page, '完成這步：兩段 NotebookLM 指令已產出');
    await waitText(page, '任務指令');
    await waitText(page, '自定義指示');
  ...
  await runStep('Test 23 LabTab mode-based next steps change', page, async () => {
    ...
    await page.locator('textarea[placeholder*="輸入內容"]').last().fill('睡眠呼吸與牙科衛教簡報');
    ...
    await page.locator('textarea[placeholder*="輸入內容"]').last().fill('一個紫色漸層卡片式提示詞');
    await clickText(page, '執行實驗');
  ...
  await runStep('Test 25 StyleStudio tab exists while Audit is unlocked', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await waitText(page, '體檢 & 增強');
    await waitText(page, '風格實驗室');
  ...
  await runStep('Test 26 Audit tab visible and clickable in seven-tab workflow', page, async () => {
    ...
    await clickText(page, '體檢 & 增強');
    await waitText(page, 'prompt 健檢工具');
    await waitText(page, '這個 tab 是做什麼的');
    await shot(page, 'sprint2-audit-tab.png');
  });

  await runStep('Test 27 Audit prompt input shows 8 dimensions and score', page, async () => {
    await clickText(page, '體檢 & 增強');
    const auditPrompt = 'Design a patient education poster for O2Win Dental for adult patients after implant surgery. Core message: good aftercare reduces swelling and infection risk. Use clean composition, strong visual hierarchy, crisp typography, white and teal palette, soft clinic lighting, portrait 1024x1536, production-quality. Include in-image text "植牙術後照護", "冰敷 24 小時", "按時回診". No fake logos, no generic card walls. Source: original clinic brief.';
    const textarea = page.locator('main textarea').first();
    await textarea.fill(auditPrompt);
    await clickText(page, '開始體檢');
    await waitText(page, 'Score');
  ...
  await runStep('Test 28 Audit grade label appears A-F', page, async () => {
    await clickText(page, '體檢 & 增強');
    const gradeText = await page.locator('main').textContent();
    expect(/Grade\s*[ABCDF]/.test(gradeText.replace(/\s+/g, ' ')) || /\b[ABCDF]\b/.test(gradeText), 'Audit 結果應顯示 A/B/C/D/F grade');
    await waitText(page, 'Grade');
  });
  ...
  await runStep('Test 30 Sprint 1.8 hidden Audit actions are now visible', page, async () => {
    ...
    await page.waitForTimeout(500);
    await page.getByRole('button', { name: /反 AI Slop 診斷/ }).last().click();
    await page.locator('textarea[placeholder*="輸入內容"]').last().fill('一個 purple gradient generic card wall poster');
    await clickText(page, '執行實驗');
    await waitText(page, '前往 體檢 & 增強');
    await shot(page, 'sprint2-audit-actions-unlocked.png');
  });
  ...
  await runStep('Test 31 Audit onboarding card, weak sample, and neutral placeholder', page, async () => {
    ...
    await clickText(page, '體檢 & 增強');
    await waitText(page, '這個 tab 是做什麼的');
    await waitText(page, '弱例');
    await waitText(page, 'stunning');
  ...
  await runStep('Test 33 Style Studio shows five category chips', page, async () => {
    ...
    await waitText(page, '極簡留白');
    await waitText(page, '薄荷奶油');
  ...
  await runStep('Test 34 selecting chips updates composed English prompt', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await page.locator('header button', { hasText: '風格實驗室' }).first().click();
    await page.locator('main input').first().fill('Dental clinic patient education poster about implant aftercare');
    await clickText(page, '瑞士國際風');
    await clickText(page, '薄荷奶油');
    const prompt = await page.locator('main textarea[readonly]').first().inputValue();
    expect(prompt.includes('Editorial poster design featuring Dental clinic patient education poster about implant aftercare'), '英文 prompt 未包含 subject 與 category lead-in');
    expect(prompt.includes('Swiss International style'), '英文 prompt 未包含瑞士國際風英文片段');
    expect(prompt.includes('soft mint green and warm cream palette'), '英文 prompt 未包含薄荷奶油英文片段');
    expect(prompt.includes('no generic card walls'), '英文 prompt 未包含 negative prompt');
  ...
  await runStep('Test 35 size and quality pickers are usable', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await page.locator('header button', { hasText: '風格實驗室' }).first().click();
    await clickText(page, '3:2 橫式');
    await clickText(page, 'medium');
    const prompt = await page.locator('main textarea[readonly]').first().inputValue();
    expect(prompt.includes('1536x1024'), '英文 prompt 未包含 3:2 尺寸');
    expect(prompt.includes('high quality, sharp details'), '英文 prompt 未包含 medium 品質英文');
    await waitText(page, '≈ $0.04');
  ...
  await runStep('Test 36 Style Studio applies to Claude Design through landingNote bridge', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await page.locator('header button', { hasText: '風格實驗室' }).first().click();
    await page.locator('main input').first().fill('Mobile app dashboard for dental clinic operations');
    await clickText(page, 'UI / 產品介面');
    await clickText(page, 'Web 桌面');
    await clickText(page, '金融科技');
    await page.getByRole('button', { name: /套用到 Claude Design/ }).last().click();
    await waitText(page, '已套用風格實驗室組好的英文 prompt');
    await waitText(page, '來源：風格實驗室');
    const textarea = page.locator('#dt-step-1 textarea').first();
    const value = await textarea.inputValue();
    expect(value.includes('Polished UI mockup of Mobile app dashboard for dental clinic operations'), 'Design textarea 未收到 Style Studio prompt');
    expect(value.includes('desktop web browser frame'), 'Design textarea 未包含 Style Studio chip 英文片段');
  ...
  await runStep('Test 37 Sprint 1.8 hidden StyleStudio actions are now unlocked', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickText(page, 'Claude Design');
    await page.locator('#dt-step-1 textarea').first().fill('做一張牙科衛教海報，主題是植牙術後照護');
    await clickText(page, '繼續 → 設定受眾與基調');
    await clickText(page, '繼續 → 選擇製圖方式');
    await clickText(page, '社群內容');
    await page.locator('#dt-step-3 textarea').first().fill('患者衛教海報，說明植牙術後照護');
    await clickText(page, '繼續 → 設定風格與生成');
    await waitText(page, '開啟風格實驗室');
    await clickText(page, '開啟風格實驗室');
    await waitText(page, '🎬 風格實驗室');
    await page.getByRole('button', { name: /送去體檢/ }).last().click();
    await waitText(page, '體檢 & 增強');
    await waitText(page, 'Score');
  ...
  await runStep('Test 45 Phase C Claude Design contains advanced Lab drawer and slide subflow', page, async () => {
    ...
    await clickText(page, '進階實驗工具');
    await waitText(page, '彩蛋 Lab');
    await waitText(page, '病毒傳播版');
    await clickText(page, '投影片簡報');
    await waitText(page, 'PPT Flow Lite');
  });
  ...
  await runStep('Test 46 Phase C neutral copy replaces dental onboarding defaults', page, async () => {
    ...
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '體檢 & 增強');
    const placeholder = await page.locator('main textarea').first().getAttribute('placeholder');
    expect(placeholder.includes('AURORA CAFE') && !placeholder.includes('O2Win Dental'), `Audit placeholder 未去醫療化：${placeholder}`);
    await clickHeaderTab(page, '風格實驗室');
    const styleInput = await page.locator('main input').first().inputValue();
  ...
  await runStep('Test 47 Phase C Truly-Neutral subject_terms does not force medical domain', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('製作紫微斗數入門課程的 10 頁簡報');
    await waitText(page, '抽取主題詞');
    const body = await page.locator('body').innerText();
    expect(body.includes('紫微斗數') && body.includes('不歸類領域'), 'Smart 應顯示 subject_terms 中性判斷');
    expect(!body.includes('領域自動選「口腔醫學」'), 'Smart 不應強制歸類口腔醫學');
  });

  await runStep('Test 48 Phase C PPT Flow Lite shows seven planning blocks', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('咖啡店開幕宣傳簡報，給附近居民看');
    await waitText(page, '簡報製作');
    await clickText(page, '確認，生成提示詞');
  ...
  await runStep('Test 49 Phase C Smart triggers PPT Flow condition path', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('幫我做 8 頁產品發表 PPT');
    await waitText(page, '任務：簡報製作');
    await clickText(page, '確認，生成提示詞');
    await waitText(page, 'PPT Flow Lite');
    await waitText(page, '瀏覽器版不直接生成 PPTX 檔');
  });
  ...
  await runStep('Test 50 Phase C NotebookLM triggers PPT Flow condition path', page, async () => {
    ...
    await clickText(page, 'NotebookLM');
    await clickText(page, '簡報製作');
    await clickText(page, '自訂領域');
    await page.locator('input[placeholder*=\"中文系教授\"]').fill('咖啡店開幕企劃');
    await clickText(page, '確認');
  ...
  await runStep('Test 51 C1 Smart shows four glow blocks simultaneously', page, async () => {
    ...
    const c1 = await page.locator('#smart-step-1').getAttribute('class');
    const c2 = await page.locator('#smart-step-2').getAttribute('class');
    const c3 = await page.locator('#smart-step-3').getAttribute('class');
    const c4 = await page.locator('#smart-step-4').getAttribute('class');
  ...
  await runStep('Test 53 C2 Step 2 shows recommended next route', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張咖啡店開幕 A4 海報圖片');
  ...
  await runStep('Test 54 C2 Step 3 shows four action cards', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('把這篇文獻整理成簡報');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    const c3 = await page.locator('#smart-step-3').getAttribute('class');
  ...
  await runStep('Test 55 C2 direct copy activates Step 4 success', page, async () => {
    ...
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張咖啡店開幕海報');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.getByRole('button', { name: /直接複製/ }).first().click();
    await waitText(page, '複製成功');
    const c4 = await page.locator('#smart-step-4').getAttribute('class');
    expect(c4.includes('glow-active'), `Step 4 應為 active glow：${c4}`);
    const copied = await page.evaluate(() => window.__formaSmartCopied || '');
    expect(copied.includes('Prompt') || copied.includes('指令'), '直接複製應寫入 prompt 文字');
    await shot(page, 'c2-direct-copy-step4.png');
  });

  await runStep('Test 56 C2 Claude Design action switches tab with landingNote', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張咖啡店開幕海報圖片');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.getByRole('button', { name: /進 Claude Design 精修/ }).first().click();
    await waitText(page, '已從 Smart 帶入 prompt');
    await waitText(page, '來源：Smart 智慧製圖');
    ...
    const value = await page.locator('#dt-step-1 textarea').first().inputValue();
    expect(value.includes('咖啡店') || value.includes('圖像生成 Prompt'), 'Design textarea 應收到 Smart prompt');
  ...
  await runStep('Test 57 C2 NotebookLM action switches tab with landingNote', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('把這篇文獻整理成 10 頁簡報');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.getByRole('button', { name: /進 NotebookLM 完整 5 step/ }).first().click();
    await waitText(page, '已從 Smart 建議進 NotebookLM 完整流程');
    await waitText(page, '來源：Smart 智慧製圖');
    const active = await page.locator('header .tab-on').first().textContent();
  ...
  await runStep('Test 58 C2 Audit action switches tab with landingNote', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張產品發表海報 prompt，要求清楚文字與版面');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.getByRole('button', { name: /送完整體檢/ }).first().click();
    await waitText(page, '來自Smart 智慧製圖的 prompt');
    await waitText(page, '來源：Smart 智慧製圖');
    await waitText(page, 'Score');
    const active = await page.locator('header .tab-on').first().textContent();
  ...
  await runStep('Test 59 C3 Smart quick audit section exists collapsed', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    const section = page.locator('#smart-quick-audit');
  ...
  await runStep('Test 60 C3 expand quick audit shows score grade pass fixes', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張產品發表海報 prompt，要求清楚文字、版面、尺寸與受眾');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    ...
    await waitText(page, '3 通過');
    await waitText(page, '3 補強');
  ...
  await runStep('Test 61 C3 apply quick fixes mutates Smart textarea', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    const smartTextarea = page.locator('textarea[placeholder*="補充說明"]').first();
    await smartTextarea.fill('做一張產品海報');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    const before = await smartTextarea.inputValue();
    await page.locator('#smart-quick-audit').getByRole('button', { name: /快速體檢/ }).click();
    await page.getByRole('button', { name: /^套用補強$/ }).click();
    const after = await smartTextarea.inputValue();
    expect(after.length > before.length, '套用補強後 Smart textarea 內容應變長');
  ...
  await runStep('Test 62 C3 send full Audit from quick audit switches tab', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張課程招生海報，包含清楚標題、受眾與尺寸');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.locator('#smart-quick-audit').getByRole('button', { name: /快速體檢/ }).click();
  ...
  await runStep('Test 64 C4 clicking Prompt Lab chip fills Smart textarea', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    const chip = page.locator('.smart-gallery-chip').first();
    await chip.waitFor({ state: 'visible', timeout: 15000 });
    await chip.click();
    const value = await page.locator('textarea[placeholder*="補充說明"]').first().inputValue();
  ...
  await runStep('Test 65 C4 Smart Step 4 shows 3-6 similar examples', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張咖啡店開幕海報，溫暖編輯風，直式');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
  ...
  await runStep('Test 66 C4 open full Prompt Lab switches tab with landingNote', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張 UI dashboard mockup');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.locator('#smart-similar-examples').getByRole('button', { name: /^開完整 Prompt Lab$/ }).first().click();
    await waitText(page, '已從 Smart 開啟完整 Prompt Lab');
    await waitText(page, '來源：Smart 智慧製圖');
    const active = await page.locator('header .tab-on').first().textContent();
  ...
  await runStep('Test 67 C5 Smart literature input shows prefilled NLM button', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('文獻整理：請把這些研究整理成摘要與簡報');
    await waitText(page, '推薦下一步路徑');
  ...
  await runStep('Test 68 C5 prefilled NLM bridge switches tab and fills state', page, async () => {
    ...
    await waitText(page, '已從 Smart 預填 NotebookLM 前 4 步');
    await waitText(page, '已從 Smart 預填，可直接到 Step 5 產生指令');
    const active = await page.locator('header .tab-on').first().textContent();
    expect(active.includes('NotebookLM'), `應切到 NLM tab，實際 ${active}`);
    await waitText(page, '文件整理');
    await waitText(page, 'IMRAD 實證框架');
  ...
  await runStep('Test 69 v2.1 pendingPayload state exists with schema variants', page, async () => {
    const source = fs.readFileSync(V2_HTML_PATH, 'utf8');
    expect(source.includes('const [pendingPayload, setPendingPayload] = useState(null);'), 'App 應使用單一 pendingPayload state');
    ...
    expect(!source.includes('setPendingDesignPrompt'), '舊 setPendingDesignPrompt 應不存在');
    expect(!source.includes('setPendingAuditText'), '舊 setPendingAuditText 應不存在');
  ...
  await runStep('Test 70 v2.1 dispatch switches targetTab and pre-fills Design', page, async () => {
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張 v2.1 schema regression poster 圖片');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.getByRole('button', { name: /進 Claude Design 精修/ }).first().click();
    await waitText(page, '已從 Smart 帶入 prompt');
    ...
    const value = await page.locator('#dt-step-1 textarea').first().inputValue();
    expect(/schema regression|圖像生成 Prompt|poster/i.test(value), 'dispatch design-prompt 應預填 Design Step 1');
  });

  await runStep('Test 71 v2.1 consume downgrades payload to note-only without replay', page, async () => {
    const source = fs.readFileSync(V2_HTML_PATH, 'utf8');
    expect(source.includes("consumePendingPayload?.('design-prompt')"), 'DesignTab 應消費 design-prompt');
    expect(source.includes("consumePendingPayload?.('audit-text')"), 'AuditTab 應消費 audit-text');
    expect(source.includes("consumePendingPayload?.('nlm-state')"), 'NLMTab 應消費 nlm-state');
    expect(source.includes("kind: 'note-only', payload: null"), 'consume 後應移除 payload，保留 note-only banner');
    expect(!source.includes('consumePendingDesignPrompt'), '舊 design consume helper 應不存在');
    expect(!source.includes('clearPendingAuditText'), '舊 audit consume helper 應不存在');
    expect(!source.includes('consumePendingNLMState'), '舊 nlm consume helper 應不存在');
  });

  await runStep('Test 72 v2.1 landingNote comes from pendingPayload.note only', page, async () => {
    const source = fs.readFileSync(V2_HTML_PATH, 'utf8');
    expect(source.includes('const landingNote = pendingPayload?.note || null;'), 'landingNote 應從 pendingPayload.note derive');
    expect(!source.includes('const [landingNote, setLandingNote]'), '舊 landingNote state 應不存在');
    expect(!source.includes('setLandingNote('), '不應再直接呼叫 setLandingNote(...)');
    await page.goto(`${BASE_URL}/forma-studio-v2.html`, { waitUntil: 'domcontentloaded' });
    await clickHeaderTab(page, '智慧製圖');
    await page.locator('textarea[placeholder*="補充說明"]').first().fill('做一張 v2.1 note bridge 海報圖片');
    await waitText(page, '推薦下一步路徑');
    await clickText(page, '確認，生成提示詞');
    await page.getByRole('button', { name: /送完整體檢/ }).first().click();
    await waitText(page, '來自Smart 智慧製圖的 prompt');
